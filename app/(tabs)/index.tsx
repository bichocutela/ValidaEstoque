...
export default function HomeScreen() {
  ...
  const { signedIn, employeeName, lots, getProduct } = useInventory();
  ...
  return <ScreenContainer className="px-5" containerClassName="bg-background">
    <FlatList
      ...
      ListHeaderComponent={<>
        ...
        <View style={styles.metrics}><MetricCard label="Produtos OK" value={normal.reduce((sum, lot) => sum + lot.currentQuantity, 0)} tone="success" icon="verified" /><MetricCard label="Próximos do vencimento" value={critical.reduce((sum, lot) => sum + lot.currentQuantity, 0)} tone="critical" icon="schedule" /><MetricCard label="Vencidos" value={expired.reduce((sum, lot) => sum + lot.currentQuantity, 0)} tone="error" icon="event-busy" /><MetricCard label="Avariados" value={damaged.reduce((sum, lot) => sum + lot.currentQuantity, 0)} tone="warning" icon="report-problem" /></View>
