...
import { requiresDuplicateLotConfirmation } from "@/lib/product-management";
...
export default function ScannerScreen() {
  ...
  const commitSave = (quantity: number) => {
  ...
  const save = () => {
    ...
    const matchingProduct = products.find((product) => (form.barcode.trim() && barcodesMatch(product.barcode, form.barcode.trim())) || (product.name.trim().toLowerCase() === form.name.trim().toLowerCase() && product.brand.trim().toLowerCase() === form.brand.trim().toLowerCase()));
    const existingLot = resolution?.existingLot ?? (matchingProduct && form.code.trim() ? lots.find((lot) => lot.productId === matchingProduct.id && lot.code.trim().toLowerCase() === form.code.trim().toLowerCase()) : undefined);
    if (existingLot && requiresDuplicateLotConfirmation(existingLot.id)) {
      Alert.alert("Lote já cadastrado", `O lote ${existingLot.code} já existe para este produto. Deseja registrar uma nova entrada mesmo assim?`, [{ text: "Revisar", style: "cancel" }, { text: "Registrar mesmo assim", onPress: () => commitSave(quantity) }]);
      return;
    }
    commitSave(quantity);
  };
